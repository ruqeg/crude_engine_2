#include <core/profiler.h>
#include <core/hash_map.h>
#include <scene/scene_components.h>
#include <graphics/scene_renderer.h>

#include <graphics/passes/gbuffer_late_pass.h>

void
crude_gfx_gbuffer_late_pass_initialize
(
  _In_ crude_gfx_gbuffer_late_pass                        *pass,
  _In_ crude_gfx_scene_renderer_frame_resources           *frame_resources,
  _In_ crude_gfx_scene_renderer_debug_resources           *debug_resources,
  _In_ crude_gfx_scene_renderer_meshes_resources          *meshes_resources,
  _In_ crude_gfx_scene_renderer_meshlets_resources        *meshlets_resources
)
{
  pass->frame_resources = frame_resources;
  pass->debug_resources = debug_resources;
  pass->meshes_resources = meshes_resources;
  pass->meshlets_resources = meshlets_resources;
  
  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    pass->meshlet_late_ds[ i ] = CRUDE_GFX_DESCRIPTOR_SET_HANDLE_INVALID;
  }

  crude_gfx_gbuffer_late_pass_on_techniques_reloaded( pass );
}

void
crude_gfx_gbuffer_late_pass_deinitialize
(
  _In_ crude_gfx_gbuffer_late_pass                        *pass
)
{
  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    crude_gfx_destroy_descriptor_set( pass->meshes_resources->renderer->gpu, pass->meshlet_late_ds[ i ] );
  }
}

void
crude_gfx_gbuffer_late_pass_render
(
  _In_ void                                               *ctx,
  _In_ crude_gfx_cmd_buffer                               *primary_cmd
)
{
  crude_gfx_gbuffer_late_pass                             *pass;
  crude_gfx_renderer                                      *renderer;
  crude_gfx_pipeline_handle                                pipeline;
 
  pass = CRUDE_REINTERPRET_CAST( crude_gfx_gbuffer_late_pass*, ctx );

  renderer = pass->meshes_resources->renderer;

  pipeline = crude_gfx_renderer_access_technique_pass_by_name( renderer, "meshlet", "meshlet" )->pipeline;
  crude_gfx_cmd_bind_pipeline( primary_cmd, pipeline );
  crude_gfx_cmd_bind_descriptor_set( primary_cmd, pass->meshlet_late_ds[ renderer->gpu->current_frame ] );
  crude_gfx_cmd_draw_mesh_task_indirect_count(
    primary_cmd,
    pass->meshes_resources->mesh_task_indirect_commands_late_sb[ renderer->gpu->current_frame ],
    CRUDE_OFFSETOF( crude_gfx_mesh_draw_command_gpu, indirect_meshlet ),
    pass->meshes_resources->mesh_task_indirect_count_late_sb[ renderer->gpu->current_frame ],
    0,
    CRUDE_ARRAY_LENGTH( pass->meshes_resources->meshes_instances ),
    sizeof( crude_gfx_mesh_draw_command_gpu )
  );
}

void
crude_gfx_gbuffer_late_pass_on_techniques_reloaded
(
  _In_ void                                               *ctx
)
{
  crude_gfx_gbuffer_late_pass                             *pass;
  crude_gfx_renderer_technique_pass                       *meshlet_pass;
  crude_gfx_descriptor_set_layout_handle                   layout;
  
  pass = CRUDE_REINTERPRET_CAST( crude_gfx_gbuffer_late_pass*, ctx );

  meshlet_pass = crude_gfx_renderer_access_technique_pass_by_name( pass->meshes_resources->renderer, "meshlet", "meshlet" );
  layout = crude_gfx_get_descriptor_set_layout( pass->meshes_resources->renderer->gpu, meshlet_pass->pipeline, CRUDE_GFX_MATERIAL_DESCRIPTOR_SET_INDEX );
  
  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    if ( CRUDE_RESOURCE_HANDLE_IS_VALID( pass->meshlet_late_ds[ i ] ) )
    {
      crude_gfx_destroy_descriptor_set( pass->meshes_resources->renderer->gpu, pass->meshlet_late_ds[ i ] );
    }
  }

  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    crude_gfx_descriptor_set_creation                    ds_creation;

    ds_creation = crude_gfx_descriptor_set_creation_empty();
    ds_creation.layout = layout;
    ds_creation.name = "meshlet_descriptor_set";
    
    crude_gfx_scene_renderer_debug_resources_add_to_descriptor_set_creation( &ds_creation, pass->debug_resources, i );
    crude_gfx_scene_renderer_frame_resources_add_to_descriptor_set_creation( &ds_creation, pass->frame_resources );
    crude_gfx_scene_renderer_meshes_resources_add_to_descriptor_set_creation( &ds_creation, pass->meshes_resources );
    crude_gfx_scene_renderer_meshlets_resources_add_to_descriptor_set_creation( &ds_creation, pass->meshlets_resources );
    crude_gfx_descriptor_set_creation_add_buffer( &ds_creation, pass->meshes_resources->mesh_task_indirect_count_late_sb[ i ], 10u );
    crude_gfx_descriptor_set_creation_add_buffer( &ds_creation, pass->meshes_resources->mesh_task_indirect_commands_late_sb[ i ], 11u );

    pass->meshlet_late_ds[ i ] = crude_gfx_create_descriptor_set( pass->meshes_resources->renderer->gpu, &ds_creation );
  }
}

crude_gfx_render_graph_pass_container
crude_gfx_gbuffer_late_pass_pack
(
  _In_ crude_gfx_gbuffer_late_pass                        *pass
)
{
  crude_gfx_render_graph_pass_container container = crude_gfx_render_graph_pass_container_empty();
  container.ctx = pass;
  container.render = crude_gfx_gbuffer_late_pass_render;
  container.on_techniques_reloaded = crude_gfx_gbuffer_late_pass_on_techniques_reloaded;
  return container;
}