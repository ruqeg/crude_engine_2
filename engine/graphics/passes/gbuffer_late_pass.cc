...
)
{
  ...
  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    ...
    crude_gfx_scene_renderer_add_meshlet_resources_to_descriptor_set_creation( &ds_creation, scene_renderer, i );
    crude_gfx_scene_renderer_add_debug_resources_to_descriptor_set_creation( &ds_creation, scene_renderer, i );
    ...
    pass->meshlet_late_ds[ i ] = crude_gfx_create_descriptor_set( scene_renderer->renderer->gpu, &ds_creation );
  }
}
...
)
{
  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    crude_gfx_destroy_descriptor_set( pass->scene_renderer->renderer->gpu, pass->meshlet_late_ds[ i ] );
  }
}
...
)
{
  ...
  crude_gfx_cmd_draw_mesh_task_indirect_count(
    primary_cmd,
    pass->scene_renderer->mesh_task_indirect_commands_late_sb[ renderer->gpu->current_frame ],
    CRUDE_OFFSETOF( crude_gfx_mesh_draw_command_gpu, indirect_meshlet ),
    pass->scene_renderer->mesh_task_indirect_count_late_sb[ renderer->gpu->current_frame ],
    0,
