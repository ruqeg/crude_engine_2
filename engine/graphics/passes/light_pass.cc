...
)
{
  ...
  light_pass = crude_gfx_renderer_access_technique_pass_by_name( scene_renderer->renderer, "fullscreen", "light" );
  light_dsl = crude_gfx_get_descriptor_set_layout( scene_renderer->renderer->gpu, light_pass->pipeline, CRUDE_GFX_MATERIAL_DESCRIPTOR_SET_INDEX );
  ...
  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    ...
    pass->light_ds[ i ] = crude_gfx_create_descriptor_set( scene_renderer->renderer->gpu, &ds_creation );
  }
}
