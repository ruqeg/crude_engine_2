...
)
{
  ...
  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    ...
    pass->culling_early_descriptor_sets_handles[ i ] = crude_gfx_create_descriptor_set( scene_renderer->renderer->gpu, &ds_creation );
  }
}
...
)
{
  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    crude_gfx_destroy_descriptor_set( pass->scene_renderer->renderer->gpu, pass->culling_early_descriptor_sets_handles[ i ] );
  }
}
...
)
{
  ...
  count_early = crude_gfx_access_buffer( gpu, pass->scene_renderer->mesh_task_indirect_count_early_sb[ gpu->current_frame ] );
  commands_early = crude_gfx_access_buffer( gpu, pass->scene_renderer->mesh_task_indirect_commands_early_sb[ gpu->current_frame ] );
  ...
  crude_gfx_cmd_bind_descriptor_set( primary_cmd, pass->culling_early_descriptor_sets_handles[ primary_cmd->gpu->current_frame ] );
