...
)
{
  ...
  debug_cube_pipeline = crude_gfx_renderer_access_technique_pass_by_name( renderer, "debug", "debug_cube" )->pipeline;
  ...
  crude_gfx_cmd_draw_inderect( primary_cmd, pass->scene_renderer->debug_commands_sb[ renderer->gpu->current_frame ], 0u, 1u, sizeof( crude_gfx_debug_draw_command_gpu ) );
  ...
  crude_gfx_cmd_draw_inderect( primary_cmd, pass->scene_renderer->debug_commands_sb[ renderer->gpu->current_frame ], CRUDE_OFFSETOF( crude_gfx_debug_draw_command_gpu, draw_indirect_2dline ), 1u, sizeof( crude_gfx_debug_draw_command_gpu ) );

  crude_gfx_cmd_bind_pipeline( primary_cmd, debug_cube_pipeline );
  crude_gfx_cmd_bind_descriptor_set( primary_cmd, pass->depth_cube_ds[ renderer->gpu->current_frame ] );
  crude_gfx_cmd_draw_inderect( primary_cmd, pass->scene_renderer->debug_commands_sb[ renderer->gpu->current_frame ], CRUDE_OFFSETOF( crude_gfx_debug_draw_command_gpu, draw_indirect_cube ), 1u, sizeof( crude_gfx_debug_draw_command_gpu ) );
}
...
)
{
  ...
  pipeline = crude_gfx_renderer_access_technique_pass_by_name( pass->scene_renderer->renderer, "debug", "debug_line3d" )->pipeline;
  debug_line3d_dsl = crude_gfx_get_descriptor_set_layout( gpu, pipeline, CRUDE_GFX_MATERIAL_DESCRIPTOR_SET_INDEX );

  pipeline = crude_gfx_renderer_access_technique_pass_by_name( pass->scene_renderer->renderer, "debug", "debug_cube" )->pipeline;
  ...
  for ( uint32 i = 0; i < CRUDE_GFX_MAX_SWAPCHAIN_IMAGES; ++i )
  {
    ...
    {
      crude_gfx_destroy_descriptor_set( pass->scene_renderer->renderer->gpu, pass->depth_cube_ds[ i ] );
    }
  }
