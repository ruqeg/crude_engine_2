...
)
{
  ...
  for ( uint32 mip_index = 0; mip_index < 0; ++mip_index )
  {
...
)
{
  ...
  depth_hierarchy_width = width = depth_texture->width / 2 + depth_texture->width % 2;
  depth_hierarchy_height = height = depth_texture->height / 2 + depth_texture->height % 2;
