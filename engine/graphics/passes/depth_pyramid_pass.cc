...
)
{
  ...
  depth_pyramid_pipeline = crude_gfx_renderer_access_technique_pass_by_name( pass->renderer, "culling", "depth_pyramid" )->pipeline;
...
)
{
  ...
  depth_resource = crude_gfx_render_graph_builder_access_resource_by_name( pass->render_graph->builder, "depth" );
  depth_texture_handle = depth_resource->resource_info.texture.handle;
  depth_texture = crude_gfx_access_texture( pass->renderer->gpu, depth_texture_handle );
  ...
  {
    crude_gfx_destroy_texture( pass->renderer->gpu, pass->depth_pyramid_texture_handle );
  }
  ...
  pass->depth_pyramid_texture_handle = crude_gfx_create_texture( pass->renderer->gpu, &depth_hierarchy_creation );

  crude_gfx_link_texture_sampler( pass->renderer->gpu, pass->depth_pyramid_texture_handle, pass->depth_pyramid_sampler );
  ...
  for ( uint32 i = 0; i < pass->depth_pyramid_levels; ++i )
  {
    if ( CRUDE_RESOURCE_HANDLE_IS_VALID( pass->depth_pyramid_views_handles[ i ] ) )
    {
      crude_gfx_destroy_texture( pass->renderer->gpu, pass->depth_pyramid_views_handles[ i ] );
    }

    depth_pyramid_view_creation.subresource.mip_base_level = i;
    pass->depth_pyramid_views_handles[ i ] = crude_gfx_create_texture_view( pass->renderer->gpu, &depth_pyramid_view_creation );
  }
...
)
{
  ...
  for ( uint32 i = 0; i < CRUDE_GFX_DEPTH_PYRAMID_PASS_MAX_LEVELS; ++i )
  {
    if ( CRUDE_RESOURCE_HANDLE_IS_VALID( pass->depth_hierarchy_ds[ i ] ) )
    {
      crude_gfx_destroy_descriptor_set( pass->renderer->gpu, pass->depth_hierarchy_ds[ i ] );
    }
  }
