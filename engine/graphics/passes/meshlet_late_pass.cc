...
void
crude_gfx_meshlet_late_pass_initialize
(
  _In_ crude_gfx_meshlet_late_pass                        *pass,
...
void
crude_gfx_meshlet_late_pass_deinitialize
(
  _In_ crude_gfx_meshlet_late_pass                        *pass
)
{
...
void
crude_gfx_meshlet_late_pass_render
(
  _In_ void                                               *ctx,
  _In_ crude_gfx_cmd_buffer                               *primary_cmd
)
{
  crude_gfx_meshlet_late_pass                             *pass;
  ...
  pass = CRUDE_REINTERPRET_CAST( crude_gfx_meshlet_late_pass*, ctx );
  ...
  if ( mesh_materials && mesh_draw_counts && mesh_draw_commands )
  {
    ...
    mesh_draw_counts->depth_pyramid_texture_index = pass->scene_renderer->depth_pyramid_early_pass.depth_pyramid_texture_handle.index;
    mesh_draw_counts->occlusion_culling_late_flag = pass->scene_renderer->occlusion_culling_late_flag = 1;
  }
...
static void
crude_gfx_meshlet_late_pass_on_resize
(
  _In_ void                                               *ctx,
...
crude_gfx_meshlet_late_pass_pack
(
  _In_ crude_gfx_meshlet_late_pass                        *pass
)
{
  ...
  container.ctx = pass;
  container.on_resize = crude_gfx_meshlet_late_pass_on_resize;
  container.render = crude_gfx_meshlet_late_pass_render;
  return container;
}