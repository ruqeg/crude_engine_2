...
)
{
  ...
  if ( CRUDE_ARRAY_LENGTH( scene_renderer->lights ) )
  {
    ...
    camera = &scene_renderer->options.camera;
    view_to_world = XMLoadFloat4x4( &scene_renderer->options.camera_view_to_world );
    ...
    for ( uint32 i = 0; i < CRUDE_ARRAY_LENGTH( scene_renderer->lights ); ++i )
    {
      ...
      XMFLOAT3 const                                      *light_translation;
  
      light = &scene_renderer->lights[ i ].light;
      light_translation = &scene_renderer->lights[ i ].translation;
      ...
      lights_gpu[ i ].position = *light_translation;
    ...
    /* Upload light to gpu */
    {
      ...
      uint32 lights_count = CRUDE_ARRAY_LENGTH( scene_renderer->lights );
      lights_tca = crude_gfx_linear_allocator_allocate( &gpu->frame_linear_allocator, sizeof( crude_gfx_light_gpu ) * CRUDE_ARRAY_LENGTH( scene_renderer->lights ) );
      crude_memory_copy( lights_tca.cpu_address, lights_gpu, sizeof( crude_gfx_light_gpu ) * CRUDE_ARRAY_LENGTH( scene_renderer->lights ) );
      crude_gfx_cmd_memory_copy( primary_cmd, lights_tca, scene_renderer->lights_hga, 0, 0 );
    }
    ...
    /* Upload light indices */
    {
      ...
      lights_indices_tca = crude_gfx_linear_allocator_allocate( &gpu->frame_linear_allocator, sizeof( uint32 ) * CRUDE_ARRAY_LENGTH( scene_renderer->lights ) );
      ...
      crude_gfx_cmd_memory_copy( primary_cmd, lights_indices_tca, scene_renderer->lights_indices_hga, 0, 0 );
    }
    ...
    /* Upload lights LUT */
    {
      ...
      crude_gfx_cmd_memory_copy( primary_cmd, lights_luts_tca, scene_renderer->lights_bins_hga, 0, 0 );
    }
    
    {
      ...
      
      {
        ...
        crude_gfx_cmd_memory_copy( primary_cmd, light_tiles_tca, scene_renderer->lights_tiles_hga, 0, 0 );
      }
    }
