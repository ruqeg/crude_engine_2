...
)
{
  ...
  
  {
    ...
    uint32 num_pools = creation->num_threads * CRUDE_GFX_SWAPCHAIN_IMAGES_MAX_COUNT;
  ...
  crude_resource_pool_initialize( &gpu->materials, gpu->allocator_container, 256, sizeof( crude_gfx_material ) );
...
)
{
  ...
  crude_resource_pool_deinitialize( &gpu->materials );
...
{
  crude_gfx_material_handle                                material_handle;
  crude_gfx_material                                      *material;

  material_handle = crude_gfx_obtain_material( gpu );
  if ( CRUDE_RESOURCE_HANDLE_IS_INVALID( material_handle ) )
  {
    return NULL;
  }
  
  material = crude_gfx_access_material( gpu, material_handle );
...
{
  crude_gfx_release_material( gpu, CRUDE_COMPOUNT( crude_gfx_material_handle, { material->pool_index } ) );
}
...
{
  return CRUDE_COMPOUNT( crude_gfx_material_handle, { crude_resource_pool_obtain_resource( &gpu->materials ) } );
}
...
{
  return CRUDE_REINTERPRET_CAST( crude_gfx_material*, crude_resource_pool_access_resource( &gpu->materials, handle.index ) );
}
...
{
  crude_resource_pool_release_resource( &gpu->materials, handle.index );
}
