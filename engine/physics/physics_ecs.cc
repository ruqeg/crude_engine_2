...
ECS_COMPONENT_DECLARE( crude_physics_kinematic_body );
ECS_COMPONENT_DECLARE( crude_physics_kinematic_body_handle );
...
CRUDE_COMPONENT_STRING_DEFINE( crude_physics_kinematic_body, "crude_physics_kinematic_body" );
CRUDE_COMPONENT_STRING_DEFINE( crude_physics_kinematic_body_handle, "crude_physics_kinematic_body_handle" );
...
)
{
  ...
  CRUDE_ECS_COMPONENT_DEFINE( world, crude_physics_kinematic_body );
  CRUDE_ECS_COMPONENT_DEFINE( world, crude_physics_kinematic_body_handle );
  ...
  CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_DEFINE( manager, crude_physics_kinematic_body );
  CRUDE_PARSE_JSON_TO_COMPONENT_FUNC_DEFINE( manager, crude_physics_kinematic_body );
  CRUDE_PARSE_COMPONENT_TO_JSON_FUNC_DEFINE( manager, crude_physics_kinematic_body );
  CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_DEFINE( manager, crude_physics_kinematic_body_handle );
}
...
CRUDE_PARSE_JSON_TO_COMPONENT_FUNC_IMPLEMENTATION( crude_physics_kinematic_body )
{
  component->type = CRUDE_CAST( crude_physics_body_shape_type, cJSON_GetNumberValue( cJSON_GetObjectItemCaseSensitive( component_json, "shape_type" ) ) );
  if ( component->type == CRUDE_PHYSICS_BODY_SHAPE_TYPE_BOX )
  {
...
CRUDE_PARSE_COMPONENT_TO_JSON_FUNC_IMPLEMENTATION( crude_physics_kinematic_body )
{
  cJSON *static_body_json = cJSON_CreateObject( );
  cJSON_AddItemToObject( static_body_json, "type", cJSON_CreateString( CRUDE_COMPONENT_STRING( crude_physics_kinematic_body ) ) );
  cJSON_AddItemToObject( static_body_json, "shape_type",cJSON_CreateNumber( component->type ) );
  if ( component->type == CRUDE_PHYSICS_BODY_SHAPE_TYPE_BOX )
  {
    cJSON *box_json = cJSON_CreateObject( );
...
CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_IMPLEMENTATION( crude_physics_kinematic_body )
{
  bool                                                     modified;
  ...
  if ( modified )
  {
    CRUDE_ENTITY_SET_COMPONENT( world, node, crude_physics_kinematic_body, { *component } );
  }
}

CRUDE_PARSE_COMPONENT_TO_IMGUI_FUNC_IMPLEMENTATION( crude_physics_kinematic_body_handle )
{
  crude_physics_kinematic_body_container                  *kinematic_body_container;
  JPH::BodyInterface                                      *jph_body_interface_class;
  JPH::RMat44                                              jph_wolrd_transform;

  kinematic_body_container = crude_physics_access_kinematic_body( manager->physics_manager, *component );

  jph_body_interface_class = &manager->physics_manager->jph_physics_system_class->GetBodyInterface( );  
  jph_wolrd_transform = jph_body_interface_class->GetWorldTransform( kinematic_body_container->jph_body_class );
...
CRUDE_ECS_SYSTEM_DECLARE( crude_physics_kinematic_body_post_simulation_system_ );
...
CRUDE_ECS_OBSERVER_DECLARE( crude_physics_kinematic_body_destroy_observer_ );
CRUDE_ECS_OBSERVER_DECLARE( crude_physics_kinematic_body_create_observer_ );
...
static void
crude_physics_kinematic_body_create_observer_
(
  _In_ ecs_iter_t                                         *it
);

static void
crude_physics_kinematic_body_destroy_observer_ 
(
  _In_ ecs_iter_t                                         *it
);
...
static void
crude_physics_kinematic_body_post_simulation_system_
(
  _In_ ecs_iter_t                                         *it
);
...
)
{
  ...
  CRUDE_ECS_OBSERVER_DEFINE( world, crude_physics_kinematic_body_create_observer_, EcsOnSet, ctx, { 
    { .id = ecs_id( crude_physics_kinematic_body ), .oper = EcsAnd }
  } );
  
  CRUDE_ECS_OBSERVER_DEFINE( world, crude_physics_kinematic_body_destroy_observer_, EcsOnRemove, ctx, { 
    { .id = ecs_id( crude_physics_kinematic_body ), .oper = EcsAnd }
  } );
...
{
  crude_physics_system_context                            *ctx;
  crude_physics_kinematic_body                            *kinematic_body_per_entity;

  CRUDE_PROFILER_ZONE_NAME( "crude_physics_kinematic_body_create_observer_" );

  ctx = CRUDE_CAST( crude_physics_system_context*, it->ctx );
  kinematic_body_per_entity = ecs_field( it, crude_physics_kinematic_body, 0 );

  for ( uint32 i = 0; i < it->count; ++i )
  {
    crude_physics_kinematic_body                          *kinematic_body;
    crude_physics_kinematic_body_handle                    kinematic_body_handle;
    crude_physics_kinematic_body_creation                  kinematic_body_creation;
    
    kinematic_body = &kinematic_body_per_entity[ i ];
    
    if ( CRUDE_ENTITY_HAS_COMPONENT( it->world, it->entities[ i ], crude_physics_kinematic_body_handle ) )
    {
      crude_physics_destroy_kinematic_body_instant( ctx->physics, *CRUDE_ENTITY_GET_IMMUTABLE_COMPONENT( it->world, it->entities[ i ], crude_physics_kinematic_body_handle ) );
    }
      
    kinematic_body_creation = crude_physics_kinematic_body_creation_empty( );
    kinematic_body_creation.type = kinematic_body->type;
    kinematic_body_creation.box.extent = kinematic_body->box.extent;
    kinematic_body_creation.mesh.handle = kinematic_body->mesh.handle;
    kinematic_body_creation.entity = it->entities[ i ];
    kinematic_body_creation.layers = kinematic_body->layers;
    kinematic_body_creation.sensor = kinematic_body->sensor;

    if ( kinematic_body_creation.type == CRUDE_PHYSICS_BODY_SHAPE_TYPE_MESH && kinematic_body_creation.mesh.handle.index == -1 )
    {
      CRUDE_ENTITY_REMOVE_COMPONENT( it->world, it->entities[ i ], crude_physics_kinematic_body_handle );
      continue;
    }
    
    kinematic_body_handle = crude_physics_create_kinematic_body( ctx->physics, &kinematic_body_creation );
    CRUDE_ENTITY_SET_COMPONENT( it->world, it->entities[ i ], crude_physics_kinematic_body_handle, { kinematic_body_handle } );
  }
}
...
)
{
  ...
  CRUDE_PROFILER_ZONE_NAME( "crude_physics_kinematic_body_destroy_observer_" );
  ...
  {
    crude_physics_kinematic_body                          *kinematic_body;
    crude_physics_kinematic_body_handle const             *kinematic_body_handle;
    
    kinematic_body = &kinematic_body_per_entity[ i ];

    kinematic_body_handle = CRUDE_ENTITY_GET_IMMUTABLE_COMPONENT( it->world, it->entities[ i ], crude_physics_kinematic_body_handle );

    if ( kinematic_body_handle )
    {
      crude_physics_destroy_kinematic_body_instant( ctx->physics, *kinematic_body_handle );
    }
  }
...
{
  crude_physics_system_context                            *ctx;
  crude_physics_kinematic_body_handle                     *kinematic_body_handle_per_entity;
  crude_transform                                         *transform_per_entity;
  JPH::BodyInterface                                      *jph_body_interface_class;

  CRUDE_PROFILER_ZONE_NAME( "crude_physics_kinematic_body_post_simulation_system_" );

  ctx = CRUDE_CAST( crude_physics_system_context*, it->ctx );
  kinematic_body_handle_per_entity = ecs_field( it, crude_physics_kinematic_body_handle, 0 );
  transform_per_entity = ecs_field( it, crude_transform, 1 );
  ...
  for ( uint32 i = 0; i < it->count; ++i )
  {
    ...
    kinematic_body_handle = &kinematic_body_handle_per_entity[ i ];
    transform = &transform_per_entity[ i ]; 
    
    kinematic_body_container = crude_physics_access_kinematic_body( ctx->physics, *kinematic_body_handle );

    node_to_world = crude_transform_node_to_world( it->world, it->entities[ i ], transform );

    XMMatrixDecompose( &scale, &rotation, &translation, node_to_world );

    jph_body_interface_class->SetPositionAndRotation( kinematic_body_container->jph_body_class, crude_vector_to_jph_vec3( translation ), crude_vector_to_jph_quat( rotation ), JPH::EActivation::Activate );
  }
cleanup:
  CRUDE_PROFILER_ZONE_END;
}