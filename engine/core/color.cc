...
{
  return uint8( r * 255.f ) | ( uint8( g * 255.f ) << 8 ) | ( uint8( b * 255.f ) << 16 ) | ( uint8( a * 255.f ) << 24 );
}
...
{
  return distinct_colors_[ index % CRUDE_COUNTOF( distinct_colors_ ) ];
}
