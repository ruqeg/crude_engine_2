...
(
  _In_ crude_scene_creation                               *creation,
...
)
{
  ...
  crude_string_buffer_initialize( &temporary_path_buffer, 1024, crude_stack_allocator_pack( temporary_allocator ) );

  json_path = crude_string_buffer_append_use_f( &temporary_path_buffer, "%s%s", creation->resources_path, creation->json_name );
  ...
  /* Parse json */
  {
    ...
    crude_read_file( json_path, crude_stack_allocator_pack( temporary_allocator ), &json_buffer, &json_buffer_size );
