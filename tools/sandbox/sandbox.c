...
)
{
  ...
  crude_entity scene = crude_entity_create_empty( sandbox->engine->world, "sandbox" );

  /* Create free camera */
  crude_entity camera = crude_entity_create_empty( sandbox->engine->world, "camera1" );
  CRUDE_ENTITY_SET_COMPONENT( camera, crude_camera, {
    .fov_radians = CRUDE_CPI4,
    .near_z = 0.01,
    .far_z = 1000,
    .aspect_ratio = 4.0 / 3.0 } );
  CRUDE_ENTITY_SET_COMPONENT( camera, crude_transform, {
    .translation = { 0, 0, -5 },
    .rotation = { 0, 0, 0, 1 },
    .scale = { 1, 1, 1 }, } );
  CRUDE_ENTITY_SET_COMPONENT( camera, crude_free_camera, {
    .moving_speed_multiplier = { 7.0, 7.0, 7.0 },
    .rotating_speed_multiplier  = { -0.25f, -0.25f },
    .entity_input = scene } );
  
  /* Create scene */
  CRUDE_ENTITY_SET_COMPONENT( scene, crude_window, { 
    .width     = 800,
    .height    = 600,
    .maximized = false });
  CRUDE_ENTITY_SET_COMPONENT( scene, crude_scene, { 0 } );
  CRUDE_ENTITY_SET_COMPONENT( scene, crude_input, { 0 } );
  CRUDE_ENTITY_SET_COMPONENT( scene, crude_gfx_graphics_creation, {
