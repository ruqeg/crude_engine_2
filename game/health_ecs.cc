...
)
{
  ...
  {
    health->callback_container.damage_callback( health->callback_container.ctx, health, damage );
  }
}
 ...
 *
 *********************************************************/
CRUDE_ECS_SYSTEM_DECLARE( crude_health_update_system_ );
...
{
  CRUDE_PROFILER_ZONE_NAME( "crude_health_update_system_" );

  crude_game                                              *game;
  crude_health_system_context                             *ctx;
  crude_health                                            *weapon_per_entity;

  game = crude_game_instance( );
  ctx = CRUDE_CAST( crude_health_system_context*, it->ctx );
  weapon_per_entity = ecs_field( it, crude_health, 0 );
