...
)
{
  ...
  for ( uint32 i = 0; i < it->count; ++i )
  {
    ...
    {
      crude_audio_device_sound_set_volume( &game->engine->audio_device, game->death_sound_handle, CRUDE_LERP( 1.5, 0, CRUDE_MAX( 0, player->time_to_reload_scene - 2 ) ) );
    }
    else
    {
      crude_audio_device_sound_set_volume( &game->engine->audio_device, player->ghost_sound_handle, 2 * pow( ( player->sanity > 0.5f ? 0.f : ( 1 -  2 * player->sanity ) ), 3.f ) );
      crude_audio_device_sound_set_volume( &game->engine->audio_device, player->heartbeat_sound_handle, 3 * ( player->health > 0.5f ? 0.f : ( 1 -  2 * player->health ) ) );
      ...
      if ( player->safe_zone_volume > 0 && player->safe_zone_volume < 1.0 )
      {
        ...
        if ( level )
        {
          crude_audio_device_sound_set_volume( &game->engine->audio_device, level->save_theme_sound_handle, CRUDE_MAX( 0.f, CRUDE_MIN( player->safe_zone_volume, 1.f ) ) );
          crude_audio_device_sound_set_volume( &game->engine->audio_device, level->ambient_sound_handle, CRUDE_MAX( 0.f, CRUDE_MIN( 1.f, 1.f - player->safe_zone_volume ) ) );
        }
      }
    }
    ...
    if ( !game->death_screen && ( player->health < 0.f || player->drug_withdrawal > 1.f || player->sanity < 0.f ) )
    {
      ...
      crude_audio_device_sound_reset( &game->engine->audio_device, game->death_sound_handle );
      crude_audio_device_sound_start( &game->engine->audio_device, game->death_sound_handle );
      crude_level_01 *level = CRUDE_ENTITY_GET_MUTABLE_COMPONENT( game->main_node, crude_level_01 );
      crude_level_boss_fight *level_boss = CRUDE_ENTITY_GET_MUTABLE_COMPONENT( game->main_node, crude_level_boss_fight );

      if ( level )
      {
        crude_audio_device_sound_set_volume( &game->engine->audio_device, level->save_theme_sound_handle, 0 );
        crude_audio_device_sound_set_volume( &game->engine->audio_device, level->ambient_sound_handle, 0 );
      }
      if ( level_boss )
      {
        crude_audio_device_sound_set_volume( &game->engine->audio_device, level_boss->background_sound_handle, 0 );
      }
      crude_audio_device_sound_set_volume( &game->engine->audio_device, player->heartbeat_sound_handle, 0 );
      crude_audio_device_sound_set_volume( &game->engine->audio_device, player->ghost_sound_handle, 0 );
    }
    ...
    if ( game->death_screen && player->time_to_reload_scene < 0 && input->keys[ SDL_SCANCODE_SPACE ].current )
    {
      ...
      crude_audio_device_sound_stop( &game->engine->audio_device, game->death_sound_handle );
    }
...
)
{
  ...
  if ( !CRUDE_ENTITY_HAS_COMPONENT( game->main_node, crude_level_boss_fight ) )
  {
    ...
    pass_options->wave_absolute_frame_scale = 100 * 0.0225;
