...
)
{
  ...
  for ( uint32 i = 0; i < it->count; ++i )
  {
    ...
    crude_gfx_model_renderer_resources_manager_get_gltf_model( &game->model_renderer_resources_manager, game->starting_room_modern_syringe_health_model_absolute_filepath, NULL );
    crude_gfx_model_renderer_resources_manager_get_gltf_model( &game->model_renderer_resources_manager, game->starting_room_modern_syringe_drug_model_absolute_filepath, NULL );
    ...
    sound_creation.positioning = CRUDE_AUDIO_SOUND_POSITIONING_RELATIVE;
    level->voiceline0_sound_handle = crude_audio_device_create_sound( &game->audio_device, &sound_creation );
    crude_audio_device_sound_start( &game->audio_device, level->voiceline0_sound_handle );

    sound_creation.absolute_filepath = game->starting_room_voiceline1_sound_absolute_filepath;
    level->voiceline1_sound_handle = crude_audio_device_create_sound( &game->audio_device, &sound_creation );

    sound_creation.absolute_filepath = game->starting_room_voiceline2_sound_absolute_filepath;
    level->voiceline2_sound_handle = crude_audio_device_create_sound( &game->audio_device, &sound_creation );
  ...
  for ( uint32 i = 0; i < it->count; ++i )
  {
    ...
    if ( level->state == CRUDE_LEVEL_STARTING_ROOM_STATE_NEED_HIT )
    {
      if ( player->sanity > 1.0 )
      {
        ...
        player->health = 0.2;
        crude_audio_device_sound_start( &game->audio_device, level->voiceline1_sound_handle );
      }
      else
      {
    ...
    {
      if ( player->health > 0.99 && !crude_audio_device_sound_is_playing( &game->audio_device, level->voiceline1_sound_handle ) )
      {
        level->state = CRUDE_LEVEL_STARTING_ROOM_STATE_NEED_DRUG;
        player->drug_withdrawal = 0.85;
        crude_audio_device_sound_start( &game->audio_device, level->voiceline2_sound_handle );
      }
      else
      {
        player->sanity = 1.0;
    ...
    {
      if ( player->drug_withdrawal < 0.01 && !crude_audio_device_sound_is_playing( &game->audio_device, level->voiceline2_sound_handle ) )
      {
        crude_audio_device_sound_start( &game->audio_device, level->voiceline3_sound_handle );
